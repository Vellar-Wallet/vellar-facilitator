...
per request, after studying a comparable live explorer
(a comparable public Stellar x402 explorer) and
...
## 0. What a comparable explorer actually built
...
- **Everyone else's traffic: the actual hard problem.** This is where
...
It reuses this repo's stack, not theirs:

| Layer | Comparable explorer | Us |
|---|---|---|
...
Reuse those structural rules (§0.2) as the starting spec — they're
...
actually probe (public facilitators). Self-serve `/announce`
...
less on a fresh, low-volume explorer than it does for a mature one's
already-running one.

### 2.6 Enrichment

Joins straight into **our own** Bazaar catalog (`src/catalog.ts`) — this is
actually a stronger position than theirs, whose enrichment depends on
...
status**, something comparable enrichment doesn't have at all. That's a real
differentiation angle, not just parity.

### 2.7 API surface (v1)

A minimal, honest subset of theirs: `/feed`, `/tx/:hash`, `/stats`,
...
a wall of "unattributed" is not a good look, and a mature explorer only earned that
...
| **3** | Full registry (seed known public facilitators) + attribution + `/facilitators`, `/sellers` directory | Attribution correctness depends on `/supported` staying truthful — same trust-but-verify posture as the rest of this codebase | M |
| **4** | Horizon backfill, `/ecosystem` views, pubnet | Public RPC rate limits/cost on pubnet (measured explicitly by others — see below) | M–L |
...
- **New hosting.** A second always-on (or scheduled-poll) service is a new
  ...
  Phases 1–3; pubnet (Phase 4) is where others had to actively measure and
  ...
  into.
- **RPC load.** Testnet: published measurements show ~1.6 transfer events/ledger,
  ...
  to known SAC contracts (USDC/EURC) is required, as comparable explorers do.
- **Storage growth.** Unbounded by default; `@libsql/client` handles this
  fine at that observed scale, but no retention/pruning policy is
...
- **Classification risk.** Structural matching can misclassify unusual
  transactions (their own comments flag several hard-won edge cases:
...
*Related: the source-level competitor audit (
