...
| **G-10** | Spend ceiling throttles honest throughput **22× earlier than sponsor exposure requires** | **open — pubnet tuning.** Accounting uses the 500,000 estimate against a measured 22,579 charge, so the ceiling refuses the 101st settle having spent ~0.23 of the 5 XLM it names. Fails safe; deliberately unchanged **Lowering the ceiling was evaluated and REJECTED (2026-09-04):** the 22x gap is between the ESTIMATE and the CHARGE, and moving the ceiling changes neither — at 1 XLM the policy admits 10,000,000/500,000 = 20 settles per window instead of 100, still spending 4.5% of what it names, so it makes throughput worse without closing the gap. It also breaks two properties asserted in `src/config.thresholds.test.ts` (the 60/min per-IP limit would exceed global capacity; the unbound pool would no longer sit below a fifth of it). The correct fix is to account at the measured charge rather than the estimate, which needs pubnet settlement data this repo does not have. **Stays open.** |
...
| **O-15** | **The verifier allows 3 seconds; the README documents a 45-second cold start. Both facts have been in this repo the whole time, and nothing checked them against each other.** Ownership verification aborts at `FETCH_TIMEOUT_MS = 3_000`, while the hosted instance's ~45s spin-up is a headline caveat in `README.md` and `using-it.md`. So Layer 2 could never verify a resource that sleeps — it succeeded only if a probe happened to fire during a warm window. Measured 2026-08-14 against `vellar-seller-demo`: **asleep → `unverifiable` at 3006ms and 4046ms; warm → `match` in 679ms**. This is what left the demo listing permanently unverified and what silently blocked displacement (see [`diagnosis-demo-listing.md`](./diagnosis-demo-listing.md)). Two correct, individually-documented facts that contradict each other in a way no test could catch, because no test ran against a sleeping host | partially closed — a timeout is now a distinct verdict from `unverifiable`, earns a **60s** cooldown rather than 15 minutes, and gets **one** bounded retry. Retries extended to **three attempts** (45s, 120s), sized from the measured 31.7s / 42.5s / 43.7s cold starts. **The underlying gap stays open and is now a documented limitation rather than a pending fix**: verification is triggered by settlement and nothing else, so a resource asleep when its last-ever payment arrives stays unverified. A periodic sweep was designed and refused — see [`decision-no-verification-sweep.md`](./decision-no-verification-sweep.md), which keeps the argument for why a timeout-only sweep *would* clear the original no-background-prober objection, and the three reasons it is still not worth building. Chief among them: it would have failed on the >200s run in O-16 too. **CAUSE CORRECTION 2026-08-14 (final):** this finding's budget-vs-cold-start gap is real and locally demonstrated (asleep → timeout at 3006ms; warm → match at 679ms), and the #58/#60 retry stands on that evidence. But its claimed role in the demo's failed displacement is **withdrawn**: the operator's log grep showed the displacement skips were the one-way latch — no probe ever ran, so no budget was ever blown for that entry. O-15 remains a genuine gap for any *unverified* sleeping resource; the demo was never in that class. See O-17 |
| **O-18** | **The badge is a boolean being asked a three-valued question — and the divergence it hides can now be permanent.** Distinct from O-17's rotation question, and it holds under every rotation option. The system has three ownership states: *never proven*, *proven and currently confirmed*, and *proven but not re-confirmed since restart*. The wire's `ownerVerified` boolean collapses the first and third — a consumer cannot tell a possible squat from a once-proven binding — while the latch treats the third as the second for displacement. Neither component misbehaves; each answers its own question per spec. The divergence between them was DESIGNED to be transient, healed by the bound owner's next settlement (`reverify` → badge restored). The demo entry broke the healing assumption: the bound owner can never settle again, so the entry is served unverified forever while enforced verified forever — no signal for consumers, maximal trust for displacement, from the same fact. Eviction does not end it either: the latch and the F3 tombstone both survive eviction by design, so there is no passive expiry of the state | **closed** — `trust.ownershipState` (`"unverified"` \| `"proven-unconfirmed"` \| `"verified"`) makes the third state expressible. Additive: `ownerVerified` is untouched, so boolean consumers see exactly what they saw; `"verified"` appears IFF the boolean is true (asserted by test), so the two fields cannot disagree; the middle value contains no substring "verified" (asserted by test, so a rename cannot silently make it promotable); `verification` clamping and `verified_only` are unchanged (asserted end-to-end). No expiry path added: the exposure is read-only, and a test drives reads + a displacement attempt + a second restart against a latched entry and asserts the latch intact throughout. The *divergence itself* remains — that is O-17's rotation question, deliberately untouched |
| **O-17** | **The one-way latch assumes "verified" means currently-verified; it can mean once-verified — and the wire hid the difference.** The demo's stale binding was PROVEN circa 2026-08-11, when `GBJX3E4G…` genuinely controlled the endpoint; the payTo then legitimately rotated, and the latch made the old proof permanent. Three settlements on 2026-08-14 were refused by the latch **correctly** — no probe ever ran — while the API served `ownerVerified: false`, because that field reports the *ephemeral badge* (RA-9 rebuilds it false on restart), not the *durable latch*, which is deliberately off the wire. Two states, one name: the rule-out table trusted the wire and excluded the actual cause. The entry is now doubly permanent and contradictory — split out as **O-18**, since it holds under every rotation option. Compounding: the G-11 boot dedup collapsed `/quote/`+`/quote` to one key and kept the *verified* entry, so the survivor is precisely the immovable one. **Cheapest discriminator, recorded for next time:** one `SELECT verified_at FROM ownership` — available the whole time — or the #57 log line once deployed; three settlements were the most expensive instrument and were used first | open — the mechanism behaved as designed; what is missing is any in-band recovery for a legitimately-rotated verified binding. Options set out, none chosen: [`decision-verified-binding-rotation.md`](./decision-verified-binding-rotation.md). Runbook §1 is the only current path |
...
- **`CATALOG_DB_AUTH_TOKEN` is in the same category as `SPONSOR_SECRET_KEY`.**
  ...
  outage.
- **Displacement has still never been observed live — and the three failed
  settlements are now explained as CORRECT behaviour.** The demo's stale
  binding was once-verified, so the one-way latch refused displacement each
  time, exactly as designed (O-17). That resolves the mystery and sharpens the
  caveat: displacement of a *genuinely unverified* binding remains unwitnessed
  outside tests, and for a *verified* binding there is no in-band path at all —
  a legitimately-rotated merchant is locked out until runbook §1 and an
  operator who is actually available. Options for that gap:
